...
If you only have simple HTML apps to expose, you could stop there and essentially be done! *(If your connector is in a Docker container, it's possible you need to add a static route or two--skip ahead to the static routing section for more info.)*

To set up SSH through our secure tunnel, we'll need to add another public hostname here. Choose a subdomain name again (I used device names: ssh-nas, ssh-erx, etc.), select ssh from the dropdown list, and enter the local IP address for that device. The official documentation says you should also add the SSH port number (:22) if the device is different than where the tunnel is installed, but I left it off and had no problems. YMMV, and test accordingly.
